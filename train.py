...
class AD_Loss(nn.Module):
    def __init__(self, frame_window) -> None:
        super().__init__()
        self.bce = nn.BCELoss()
    ...
    def forward(self, result, _label):
        ...
        b = _label.size(0)//2
        t = att.size(1)      
        anomaly = torch.topk(att, t//self.frame_window + 1, dim=-1)[0].mean(-1)
        anomaly_loss = self.bce(anomaly, _label)

        panomaly = torch.topk(1 - N_Aatt, t//self.frame_window + 1, dim=-1)[0].mean(-1)
        panomaly_loss = self.bce(panomaly, torch.ones((b)).cuda())
        
        A_att = torch.topk(A_att, t//self.frame_window + 1, dim = -1)[0].mean(-1)
